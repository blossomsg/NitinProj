...
class VacuumCleanerDelegate(vacuum_cleaner_view_ui.VacuumCleanerViewUI):
    ...
    def vacuum_cleaner_textedit_values(self):
        ...
        self.vacuum_cleaner_textedit_wid.setText(str(self.vacuum_cleaner_load_techcheck_json("default_techcheck.json").get(self.vacuum_cleaner_listwid.currentItem().text())))
    ...
    def vacuum_cleaner_setup(self):
        ...
        self.vacuum_cleaner_listwid.addItems(self.vacuum_cleaner_load_techcheck_json("default_techcheck_updated.json").keys())
    ...
    def vacuum_cleaner_textedit_values_updated(self):
        # print self.vacuum_cleaner_listwid.currentItem().text(), "---->", self.vacuum_cleaner_load_techcheck_json("default_techcheck.json").get(self.vacuum_cleaner_listwid.currentItem().text())
        self.vacuum_cleaner_textedit_wid.setText(str(self.vacuum_cleaner_load_techcheck_json("default_techcheck_updated.json").get(self.vacuum_cleaner_listwid.currentItem().text())))
    ...
    def write_updated_techcheck_josn(self, var_name_with_new_values):
        with open(os.path.join("E:\\Proj_Codes\\NitinProj\\vacuum_cleaner\\techcheck_func\\", "default_techcheck_updated.json"), 'w') as techcheck_func_dic:
    ...
    def camera_exists(self, updated_techcheck_attr):
        ...
        if "renderCamShape" in camera_list:
            listview_update_attr[updated_techcheck_attr] = "Camera exists in the scene with the right name - renderCamShape"
            ...
            listview_update_attr[updated_techcheck_attr] = "Camera does not exists in the scene or the name of the camera is improper kindly rectify name with renderCamShape(name the transform node to camRender automatically will rename the shapenode)"
            self.write_updated_techcheck_josn(listview_update_attr)
